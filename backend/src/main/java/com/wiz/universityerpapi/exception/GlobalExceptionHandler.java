...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponseDTO> handleBadCredentialsException(Exception ex) {
        return buildError(HttpStatus.UNAUTHORIZED, "UNAUTHORIZED", "Sai tài khoản hoặc mật khẩu");
    }

    @ExceptionHandler(AccessDeniedException.class)
    public ResponseEntity<ErrorResponseDTO> handleAccessDeniedException(AccessDeniedException ex) {
        log.warn("Truy cập bị từ chối: {}", ex.getMessage());
        return buildError(HttpStatus.FORBIDDEN, "FORBIDDEN", "Bạn không có quyền thực hiện hành động này");
    }
    ...
    public ResponseEntity<ErrorResponseDTO> handleMalformedJson(HttpMessageNotReadableException ex) {
        ...
        return buildError(HttpStatus.BAD_REQUEST, "MALFORMED_JSON", "Request body không hợp lệ hoặc sai định dạng JSON");
    }
    ...
    public ResponseEntity<ErrorResponseDTO> handleConflictException(ConflictException ex) {
        ...
        return buildError(HttpStatus.CONFLICT, "DATA_CONFLICT", ex.getMessage());
    }
    ...
    public ResponseEntity<ErrorResponseDTO> handleEntityNotFoundException(EntityNotFoundException ex) {
        ...
        return buildError(HttpStatus.NOT_FOUND, ErrorCode.ENTITY_NOT_FOUND.name(), ErrorCode.ENTITY_NOT_FOUND.getMessage());
    }
    ...
    public ResponseEntity<ErrorResponseDTO> handleBusinessException(BusinessException ex) {
        ...
        return buildError(status, ex.getErrorCode().name(), ex.getMessage());
    }
    ...
    public ResponseEntity<ErrorResponseDTO> handleBusinessRuleViolationException(BusinessRuleViolationException ex) {
        ...
        return buildError(HttpStatus.UNPROCESSABLE_ENTITY, "BUSINESS_RULE_VIOLATION", ex.getMessage());
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ErrorResponseDTO> handleGeneralException(Exception ex, HttpServletRequest request) {
        log.error("Unhandled exception tại [{}]: {}", request.getRequestURI(), ex.getMessage(), ex);
        return buildError(HttpStatus.INTERNAL_SERVER_ERROR, "INTERNAL_SERVER_ERROR", "Hệ thống đang gặp sự cố. Vui lòng thử lại sau hoặc liên hệ quản trị viên.");
    }
